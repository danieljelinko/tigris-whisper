...
## In flight
- New-models Phase 2 (Linux+NVIDIA GPU) **code-complete & green** (pytest 71): `transformers_cuda` (Cohere/Omnilingual) + `nemo_cuda` (Nemotron) backends; picker now lists all new models on feynman (linux_cuda). **Remaining: real GPU end-to-end** — heavy deps pull via `uv run --with` on first launch (multi-GB), so verify Nemotron 0.6B (NeMo) + Cohere 2B (transformers pipeline) actually load+transcribe; the `transformers.pipeline(...)` path for Cohere/Omnilingual may need `trust_remote_code`/a specific model class — confirm on first run and patch the server if so.
...
## Next
- New-models Phase 2 real GPU verify on feynman: launch Nemotron (`nemo_cuda`) + Cohere (`transformers_cuda`), confirm load+transcribe; patch server loader if `transformers.pipeline` needs `trust_remote_code`/a specific class.
