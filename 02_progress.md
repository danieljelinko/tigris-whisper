...
## In flight
- **Cohere `crispasr` (Linux CPU) VERIFIED on feynman** — real transcript `'Testing whisper transcription. 1, 2, 3, 4, 5.'` (q4_k GGUF). Caveat: the prebuilt binary needs `libopenblas0`/`libgfortran5` (apt, or drop into `~/.cache/crispasr/lib`).
- **Omnilingual re-routed to a dedicated `omnilingual` backend** (Meta's fairseq2 `omnilingual-asr`) — the old `transformers_cuda` route failed: `omniASR` has no `model_type`, so `transformers.pipeline` can't load it (user hit this with `omniASR-LLM-7B`, which also needs ~17 GB — won't fit 8 GB). Backend + catalog re-route landed & unit-green; **real GPU load/transcribe with 300M still to verify** (omnilingual-asr API used per docs).
- Nemotron `nemo_cuda` **VERIFIED** end-to-end on the RTX 5070. Cohere `transformers_cuda` (GPU) still unverified (CPU CrispASR path verified instead).
...
## Next
- Verify the `omnilingual` backend with 300M on the GPU (real load+transcribe via `uv run --with omnilingual-asr`); confirm the `ASRInferencePipeline(...).transcribe([wav], lang=[code])` shape matches the server.
- Optional: verify Cohere `transformers_cuda` on the GPU (CPU CrispASR path already verified).

## Done
| Date | Task | Verified by |
| 2026-06-26 | Omnilingual fix: dedicated `omnilingual` backend (Meta fairseq2 `omnilingual-asr`, `ASRInferencePipeline`) replaces the broken `transformers_cuda` route (omniASR has no `model_type` → pipeline can't load it). New `src/omnilingual_asr_server.py` (lazy import, ISO→`{lang}_{script}` map incl. hu→hun_Latn) + `backend_omnilingual.sh` (`uv run --with omnilingual-asr`) + run.sh case; catalog re-routed to library `model_card` ids (`omniASR_{LLM,CTC}_<size>[_v2]`) with VRAM in labels; `IMPLEMENTED_BACKENDS` + benchmark wired. | `uv run pytest` 77 passed/1 skipped; `WHISPER_BACKEND=omnilingual run.sh --print-backend`; catalog test asserts linux_cuda→omnilingual. **Real GPU run pending.** |
| 2026-06-26 | Cohere `crispasr` (Linux CPU) VERIFIED end-to-end; hardened the lib dep. `install_crispasr.sh` bundles tarball `.so`s + runs an `ldd` check + prints the `apt install libopenblas0 libgfortran5` fix; `backend_crispasr.sh` adds `~/.cache/crispasr/lib` to `LD_LIBRARY_PATH`; contract test skips unless the binary actually executes; `test_run_dispatch.sh` isolates from a local `tigris-whisper.env`. | feynman: `crispasr --backend cohere -m auto -f sample_speech.wav` → `'Testing whisper transcription. 1, 2, 3, 4, 5.'`; `uv run pytest` 77/1-skip; dispatch 6/6 |
