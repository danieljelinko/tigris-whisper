...
## Next
- Phase 2 follow-up: verify Cohere/Omnilingual `transformers_cuda` on the GPU (real load+transcribe); patch `get_pipe()` if `trust_remote_code`/a non-pipeline loader is needed.
