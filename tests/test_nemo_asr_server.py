"""Contract test for the NVIDIA NeMo ASR HTTP wrapper (Nemotron 3.5 on Linux+GPU).

NeMo + torch are heavy and GPU-bound, so we mock transcription at its boundary
(`transcribe_audio`) and verify the wrapper honours the daemon's HTTP contract:
`POST /v1/audio/transcriptions` (multipart file) → JSON `{"text": ...}`, and
`GET /` → `{"model": ...}`.
"""
import io
import pytest

import nemo_asr_server as srv


@pytest.fixture
def client(monkeypatch):
    "Flask test client with the NeMo boundary stubbed out."
    monkeypatch.setattr(srv, "transcribe_audio", lambda path, language=None: "hello from nemo")
    app = srv.create_app()
...
def test_transcribe_returns_json_text_for_uploaded_wav(client):
    ...
    assert resp.get_json()["text"] == "hello from nemo"
